...
from utils.generic_utils import load_dataset_at, calculate_dataset_metrics, cutoff_choice, \
                                cutoff_sequence
from utils.constants import MAX_NB_VARIABLES, MAX_TIMESTEPS_LIST, NB_CLASSES_NAMES
import ipdb
...
def train_model(model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
...
def evaluate_model(model, dataset_id, dataset_prefix, dataset_fold_id=None, batch_size=128, test_data_subset=None,
    ...
    plot_confusion_matrix.run(y_test, y_pred, class_names=NB_CLASSES_NAMES[dataset_id])
...
def compute_average_gradient_norm(model, dataset_id, dataset_fold_id=None, batch_size=128,
